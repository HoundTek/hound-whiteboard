/**
 * @file I/O Roundtrip 性能测试
 * @module benchmarks/io-roundtrip
 */
...
function createFixture() {
  const rootPath = fs.mkdtempSync(path.join(os.tmpdir(), "hound-io-roundtrip-"));
...
async function run() {
  ...
  try {
    ...
    const results = await window.webContents.executeJavaScript(
      `import("./io-roundtrip-renderer.js").then((module) => module.runRoundtripBenchmark(${JSON.stringify({ rootPath: fixture.rootPath, iterations: ITERATIONS })}))`,
      true,
    );

    console.log("开始 I/O Roundtrip 性能测试...\n");
    console.log("═══════════════════════════════════════════════════");
...
app.whenReady()
  ...
  .then(() => app.quit())
  .catch((error) => {
    console.error("I/O Roundtrip benchmark failed:", error);
    app.exit(1);
  });