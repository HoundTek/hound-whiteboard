...
在向白板中添加对象 $\mathbf{a}$ 时，分两步完成：

1. **加入动态图**：在 $\mathbf{D}$ 中创建新层 $(\text{active}, \{A\}, \varnothing)$，顺序在最上方。
2. **提交**：将 $\mathbf{a}$ 作为活动对象提交（参见下文“提交活动对象”），此时 $A$ 从 $\mathbf{D}$ 移入 $\mathcal{S}$，并根据相交关系写入对应的边。

其实现见 [active-object-manager-document.md](./active-object-manager-document.md) 的“加入白板外对象”与“提交并取消选择”两节。

### 在白板中删除对象

将指定对象 $\mathbf{a}$ 从动态状态图中移除：找到 $A$ 所在层，将 $A$ 从该层的 $\mathbb{A}_i$ 中删去。若该层 $\mathbb{A}_i$ 因此变为空集，则将 $s_i$ 标记为 $\text{inactive}$。最后执行动态图清理，由下层向上删除位于最下面 active 层之下的 inactive 层。

注意：该操作仅在 $\mathbf{D}$ 层面处理，$\mathcal{S}$ 中对应 $A$ 的节点和边由下层组件负责清理。

其实现见 [active-object-manager-document.md](./active-object-manager-document.md) 的“从白板删除并取消选择”一节。

### 在白板上选择对象

将被选择的对象所对应的点集记为 $\mathbb{A}$（活动点集）。
...
按层数将 $\mathcal{G}$ 中的点分配到新的层 $(\text{active}, \mathbb{A}_i, \mathcal{D}_i)$：
...
首先，对于每个 $\mathbf{a} \in \mathbb{A}$，从不再覆盖的旧区块 $\mathcal{S}$ 中移除节点 $A$ 和关联边，并清空当前覆盖区块中 $A$ 的所有旧边。

然后，对每个 $\mathbf{a} \in \mathbb{A}$：
...
将指定对象集 $\mathbb{A}$ 中各对象从 $\mathbf{D}$ 中其当前所在的活动层移除，执行动态图清理，再为 $\mathbb{A}$ 中各对象按其原有层间关系在 $\mathbf{D}$ 中创建新层并置于顶部。
