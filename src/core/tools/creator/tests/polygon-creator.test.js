...
describe("PolygonCreatorTool", () => {
  test("PolygonCreatorTool 应在同一手势内更新当前顶点，并在 end 时固化", () => {
    ...
    const deviceContext = { context: {}, objectId: 10, ownerChunkId: 1 };
  ...
  test("构造参数应允许通过 property 指定新建多边形属性", () => {
    ...
    tool.process(
      ...
      { context: {}, objectId: 99, ownerChunkId: 1 },
    );
  ...
  test("object-cancel 信号应取消整个多边形对象并撤销 AOM 注册", () => {
    ...
    const deviceContext = { context: { board }, objectId: 10, ownerChunkId: 1 };
  ...
  test("object-end 后应将对象交给 activeObjectManager.apply", () => {
    ...
    tool.process(
      ...
      { context: { board }, objectId: 10, ownerChunkId: 1 },
    );
  ...
  test("顶点更新前后应记录旧几何快照并请求活动层刷新", () => {
    ...
    tool.process(
      ...
      { context: { monitor }, objectId: 31, ownerChunkId: 1 },
    );
  ...
  test("真实 Board 上 object-end 后应经由 AOM.apply 落回归属区块", () => {
    ...
    tool.process(
      ...
      { context: { board }, objectId: 23, ownerChunkId: 1 },
    );
