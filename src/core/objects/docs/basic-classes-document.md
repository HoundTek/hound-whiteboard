...
| `setData(data)`                               | 批量更新持久化数据，自动触发派生重算 | `(Object) => void`                                |
| `appendListItem(key, item)`                   | 向列表型字段追加一项                 | `(string, any) => void`                           |
...
所有数据修改都应通过以下四个方法，不要直接操作 `this.data` 的子字段：

| 方法                                     | 用途              | 示例                                   |
| ---------------------------------------- | ----------------- | -------------------------------------- |
| `setData({ radius })`                    | 批量写入/覆盖字段 | `obj.setData({ radius: 10 })`          |
| `appendListItem('points', { x, y })`     | 列表追加          | `obj.appendListItem('points', pt)`     |
