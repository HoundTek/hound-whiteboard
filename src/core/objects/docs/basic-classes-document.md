...
| `boundingBox`     | 对象的包围盒范围                                                                              | `RectangleRange`  |
| `property`        | 对象属性字典，存放颜色、描边宽度、字体等渲染与行为属性                                        | `Object`          |
| `center`          | 对象的几何中心                                                                                | `Vector`          |
| `convexHullRange` | 对象的凸包范围                                                                                | `Range`           |
| `rotateCenter`    | 对象的旋转中心                                                                                | `Vector`          |
| `isErasable()`    | 是否是可擦对象                                                                                | `void -> boolean` |
| `isDirected()`    | 是否是有向对象                                                                                | `void -> boolean` |
...
| `getRange()`             | 获取对象的主判定范围               | `void -> Range`                       |
| `setProperty(property)`  | 合并对象属性                       | `Object -> Object`                    |
| `getRenderPadding()`     | 从对象属性动态推导渲染留白         | `void -> number`                      |
| `isErasable()`           | 返回对象是否可擦                   | `void -> boolean`                     |
| `isDirected()`           | 返回对象是否有向                   | `void -> boolean`                     |
...
所有可持久化对象的 `serialize()` 都应返回一个可直接转成 JSON 的普通对象，并至少包含以下字段：

| 名称           | 描述                                       |
| -------------- | ------------------------------------------ |
| `type`         | 对象类型标识，用于统一反序列化分发         |
| `id`           | 对象 id                                    |
| `ownerChunkId` | 对象所在区块 id                            |
...
推荐做法是将持久化数据交给其中的 `deserializer(data)`，而不是在业务代码里手动根据 `type` 分支调用某个对象类的 `parse()`。
...
- 所有可调的渲染属性都应存放在 `property` 字段，而不是散落在 `color`、`textProperty` 这类独立字段里
- `property` 的具体键由对象类型自行定义，例如 `StrokeObject.property.width`、`CircleObject.property.strokeWidth`、`TextObject.property.font`
- `getRenderPadding()` 默认会从 `property.strokeWidth`、`property.width`、`property.outlineWidth` 这类宽度字段动态推导额外留白
- 对文本这类属性变化会影响几何富数据的对象，应该通过对象提供的 setter 入口更新 `property`，而不是直接原地修改其中某个键

这意味着 renderer 不再依赖对象族各自硬编码的 padding 常量，而是依赖对象当前 `property` 的真实宽度配置。
