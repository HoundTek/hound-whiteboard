 ...
 * @module core/ui/prefixs/multi-tool-handler
 * @author Zhou Chenyu
 */
 ...
 * @property {Object} [acc] - 追加到累积上下文的键值对（通常放入回调函数）
 */
 ...
 * @property {import("../devices-dag/dag.js").DevicesDAGHandlerContext} prefixContext - DAG 处理器上下文，含 setState、routeToChild、stop、delNodeState 及 acc
 */
...
function createMultiToolPrefixHandler(options = {}) {
  ...
  return createPrefixNodeHandler({
    ...
    handle(packet, prefixContext = {}) {
      ...
      // 将 transition.acc（回调函数等）传递到下游子节点的累积上下文
