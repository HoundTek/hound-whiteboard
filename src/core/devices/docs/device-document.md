...
当前实现里，设备内部已经使用 EventBus 作为信号传输通道：设备接收一个信号包，经工具链处理后，再把输入包和输出包分别作为事件发出。
...
设备树本身的节点、路径与分发规则，可继续参考[设备树文档](./devices-tree-document.md)。
...
在当前代码实现中，这种“间接传递”被拆成两段：

- Device 内部用 EventBus 传递和观察信号包。
- DevicesTree 负责按路径将信号包送到目标节点。

因此，设备既不是直接互调，也不是把路由逻辑硬编码到工具里。
