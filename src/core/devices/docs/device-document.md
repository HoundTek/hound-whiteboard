...
当前代码中已经提供了一个鼠标设备示例，见 [mouse-device.js](../mouse-device.js)。它维护主键是否处于激活状态，并把输入路由到两个子节点之一：

- `/hover`：没有主键按下时的移动输入
- `/primary`：主键按下、拖动以及抬起结束时的输入

这使得同一个鼠标设备可以把“悬停逻辑”和“拖动工具逻辑”拆到两个不同节点上，而不必在单个处理器里硬塞所有分支。
...
- `/keyup`：接收抬起事件
- `/repeat`：接收长按重复事件
- `/code/<KeyCode>`：接收某个具体键位的事件，如 `Space`、`KeyW`

这样做的意义是：

- Monitor 侧可以在通用节点消费视口操作键
- 工具侧可以在具体键位节点消费用户绑定的按键
