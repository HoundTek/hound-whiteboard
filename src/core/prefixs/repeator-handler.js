...
function createRepeatorPrefixHandler(options = {}) {
  ...
  /**
   * 解析目标子节点列表
   * @param {import("../devices/devices-tree.js").DevicesTreeHandlerContext} prefixContext - 当前修饰节点上下文
  ...
  return createPrefixNodeHandler({
    handle(packet, prefixContext) {
      ...
      const cloned = cloneFn(packet.signals);
      return targets.map((childName) =>
        prefixContext.routeToChild(childName, cloned),
      );
    },
  });
}
