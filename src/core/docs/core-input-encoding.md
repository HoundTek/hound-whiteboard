...
进入键盘设备后，设备树节点还可以继续把原始 `keydown` / `keyup` 改写成更稳定的设备语义信号。这个能力现在不是键盘设备私有逻辑，而是 `DevicesTreeNode` 的通用配置：节点可声明 `rewritePacket`。例如某个按键节点可以把 `trigger` 改写为一条 `position`，再在返回包里显式写入公共工具节点路径。
