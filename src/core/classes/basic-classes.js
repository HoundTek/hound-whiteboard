...
class BasicObject {
  ...
  /**
   * 标识对象是否是有向对象
   * @description 有向对象可以自定义旋转中心。在构造函数处设置。
   * @private
   * @type {boolean}
   * @readonly
   */
  ...
  /**
   * 该对象是否是可擦对象
   * @description 可擦对象可以被橡皮擦工具擦除。在构造函数处设置。
   * @private
   * @type {boolean}
   * @readonly
   */
  ...
  /**
   * 创建一个新的基础对象
   * @param {Point} position - 对象的初始位置
   * @param {number} id - 对象 id
   * @param {number} onPage - 对象所在页的 id
   * @param {boolean} [erasable = false] - 对象是否为可擦对象
   * @param {boolean} [directed = false] - 对象是否为有向对象
   * @constructor
   */
  constructor(position, id, onPage, erasable = false, directed = false) {
...
class OneDimensionObject extends BasicObject {
   ...
   * @constructor
   */
  constructor(position, id, onPage, erasable = false, directed = false, xAxis = true) {
    super(position, id, onPage, erasable, directed);
