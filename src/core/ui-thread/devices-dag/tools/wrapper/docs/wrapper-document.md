...
| `multi-tool-wrapper.js` | `MultiToolWrapper`    | 多指并发分流（动态 node 槽位，见下文"关系"） |
...
`MultiToolWrapper`（多指并发分流）同样建在 `WrapperTool` 基座之上，但使用的是**动态 node 槽位**，与 handoff / switcher 的**静态 tool 槽位**相对：

|          | 静态 tool 槽位（handoff / switcher）      | 动态 node 槽位（multi-tool）                   |
| -------- | ----------------------------------------- | ---------------------------------------------- |
