...
- 路由参数上下文通过 handler 返回值 `routeContext` 逐层追加，不能覆盖已有键
- `acc` 作为兼容旧接口的合并视图保留（等价于 `services + routeContext`）
- 需要可变共享数据时使用节点 `state`
...
| `routeContext` | `Object`         | 要追加到路由参数上下文的键值对      | 合并到路由参数上下文供下游节点读取；已有键重复则抛错                 |
| `acc`          | `Object`         | 兼容旧接口别名，等价于 routeContext | 兼容旧接口的合并视图（`services + routeContext`）                    |
...
- `signalPacket`：当前信号包
- `services`：静态服务上下文
- `routeContext`：路由参数上下文
- `acc`：兼容旧接口的合并视图（等价于 `services + routeContext`）
...
- `routeContext` 是逐层追加的只读视图。上游节点可通过返回 `{ routeContext: { key: value } }` 注入参数，下游节点只能读取，不能覆盖已有键。
- `acc` 仅作为旧接口兼容快照保留，新代码优先使用 `services` 与 `routeContext`。
- 需要可变共享数据时，应显式写入节点 `state`。例如对象桥接、拖拽锚点、局部状态机都应落在节点状态里。

如果一条链路需要"向上通知"，推荐做法是使用事件钩子（如 `action:complete`），或在 `routeContext` 中传递回调函数。
...
| **路由参数上下文** | `handlerContext.routeContext` | 当前分发链路                  | 上游注入，下游只读；不可覆盖已有键 | 单次 `dispatch`      |
...
8. 同一节点可被多条路径命中，但单次 dispatch 的 `acc`（兼容旧接口） 只沿当前路由链累积。
