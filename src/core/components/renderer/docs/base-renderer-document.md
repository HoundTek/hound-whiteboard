...
静态层渲染器用于把 `MonitorCore.chunkLoader` 当前已加载区块中的静态对象，按"已加载区块合并后的全局静态图拓扑序"绘制到 Worker 侧 base OffscreenCanvas。
BaseRenderer 仅运行在 Worker 线程（通过 `MonitorCore`），UI 侧不持有对应的 base DOM canvas。
...
- `MonitorCore.chunkLoader` 负责回答“当前视口缓冲区里有哪些区块”
