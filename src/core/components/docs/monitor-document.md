...
当前设备树由 Board 持有，Monitor 只提供挂载代理入口，不再拥有独立树实例。

业务侧挂载子树时，应优先通过 monitor.mountSubTree(subTreeDefinition) 或 monitor.mountSubTree(pathPrefix, subTreeDefinition) 进入。这个入口会把相对当前 Monitor 的路径补全后，再转交给底层 board.devicesTree.mountSubTree(basePath, subTreeDefinition)。

业务侧挂载工具时，应显式写到最终叶子路径，例如 /keyboard/code/KeyW/tool、/keyboard/code/Space/create-circle/params/tool 或 /mouse/pointer/tool，再通过 monitor.mountTool(path, tool) 进入。

这里有一条新的路径约束：

- 键盘相关 workflow 更推荐直接挂在 `/keyboard/code/<Key>/...` 的后代
- Monitor 负责补上 monitorId，不负责替业务修正跨层回跳路径
- 若某个节点需要公共逻辑，应显式挂公共子树，而不是让子节点依赖 `../../tools/...`
...
- 已实现：设备树挂载入口、世界坐标与区块坐标换算、多层画布骨架、`BaseRenderer` 挂载、`baseRenderScheduler` 挂载、`RenderScheduler` 挂载、`uiRenderScheduler` 挂载、`LiveRenderer` 挂载、`UiRenderer` 挂载、`worldRectToScreenRect()`。
