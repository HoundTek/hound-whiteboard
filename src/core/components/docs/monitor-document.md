...
业务侧挂载设备或其它输入子树时，应优先通过 monitor.mountDevice(subTreeDefinition) 或 monitor.mountDevice(pathPrefix, subTreeDefinition) 进入。这个入口会把相对当前 Monitor 的路径补全后，再转交给底层 board.devicesTree.mountDevice(basePath, subTreeDefinition)。
