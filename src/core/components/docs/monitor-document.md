...
业务侧挂载设备时，应优先通过 monitor.mountDevice(deviceDefinition) 或 monitor.mountDevice(pathPrefix, deviceDefinition) 进入。这个入口会把相对当前 Monitor 的路径补全后，再转交给底层 board.devicesTree.mountDevice(basePath, deviceDefinition)。

业务侧挂载工具时，应显式写到最终叶子路径，例如 /keyboard/tools/move/tool 或 /mouse/pointer/tool，再通过 monitor.mountTool(path, tool) 进入。
...
- 已接入：活动层 dirty rect 已可沿 `monitor.renderScheduler -> liveRenderer.flush(dirtyRects)` 这条链路执行；静态层 dirty rect 已可沿 `monitor.baseRenderScheduler -> baseRenderer.flush(dirtyRects)` 这条链路执行；区块缓冲区变化与视口变化也会自动触发 base 层刷新，并同时覆盖旧区块与新区块；视口平移/中心缩放/全屏 flush 已有显式 API；memory 白板下的 chunk buffer 已切到增量扩展策略。
