...
5. 为对象计算屏幕包围盒，并叠加对象自身的 `getRenderPadding()` 留白
...
- 已实现：按已加载区块收集静态对象、按已加载区块合并后的全局静态图拓扑序绘制、跨区块重复对象去重、显式 dirty rect 局部清理与局部重绘、对象级 `getRenderPadding()`、对象级静态失效、批内 dirty rect 合并。
