...
- `liveCanvas`：唯一显示面（可见）。先通过 `drawImage` 拷贝 `baseCanvas` 像素作为静态背景，再绘制活动对象到上层
...
3. 按 AOM 层顺序绘制活动对象到 `liveCanvas`

全量刷新与脏区刷新走同一套流水线，区别在于第一步和第二步的作用范围：

| 模式 | clear | copyBase | render |
|------|-------|----------|--------|
| 全量 | `clear()` 整画布 | `copyBase()` 全量拷贝 | 全部活动对象 |
| 脏区 | `clearDirtyRects()` 局部 | `copyBaseRects()` 脏区拷贝 | 命中脏区的活动对象 |
...
- `baseCanvas` 只包含静态对象（AOM 对象已被 `BaseRenderer` 过滤），拷贝结果天然不含活动对象
