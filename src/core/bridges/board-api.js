 ...
 * @description
 * 提供 BoardApi 的同线程实现与 RPC 实现。
 * P2 阶段默认使用同线程封装（createObject 返回 Promise.resolve(id) 而非真实异步），
 * P3 阶段补充 BoardApiRpc，通过 postMessage 与 Core Worker 通信，并保持相同方法签名。
 ...
 * 方法签名与同线程 BoardApi 保持一致，但不再直接暴露 BoardCore 引用。
 * @author Zhou Chenyu
 */
class BoardApiRpc {
  /**
